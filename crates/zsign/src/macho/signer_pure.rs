//! Pure Rust Mach-O signing implementation
//!
//! This module provides signing functionality using pure Rust cryptography,
//! mirroring the OpenSSL-based signer.rs but using PureRustCredentials.
...
#[cfg(feature = "pure-rust")]
use crate::crypto::cms_pure;
#[cfg(feature = "pure-rust")]
use crate::crypto::{PureRustCredentials, cert::SigningKeyType};
#[cfg(feature = "pure-rust")]
use crate::Result;
#[cfg(feature = "pure-rust")]
...
/// Sign a Mach-O binary using pure Rust cryptography.
///
...
/// * `credentials` - Pure Rust signing credentials
...
#[cfg(feature = "pure-rust")]
pub fn sign_macho_pure(
    macho: &MachOFile,
    identifier: &str,
    entitlements: Option<&[u8]>,
    credentials: &PureRustCredentials,
    ...
    let signed = sign_slice_complete_pure(
        slice_data,
        slice,
        identifier,
        entitlements,
...
pub fn sign_macho_all_slices_pure(
    ...
    for (index, slice) in macho.slices().iter().enumerate() {
        ...
        let mut signed = sign_slice_complete_pure(
            slice_data,
            slice,
            identifier,
            entitlements,
...
/// Sign a single slice using pure Rust and return complete signed binary data.
#[cfg(feature = "pure-rust")]
fn sign_slice_complete_pure(
    slice_data: &[u8],
    slice: &ArchSlice,
    identifier: &str,
    entitlements: Option<&[u8]>,
    ...
    let subject_cn = extract_subject_cn_pure(&credentials.certificate).unwrap_or_default();
    ...
    let preliminary_sig = build_superblob_pure(
        preliminary_code,
        slice,
        identifier,
        team_id,
    ...
    let final_sig = build_superblob_pure(
        &code_for_hashing,
        &working_slice,
        identifier,
        team_id,
    ...
    let signed_data = embed_signature_into_prepared_pure(&prepared_code, &final_sig, sig_offset, target_binary_size);
...
#[cfg(feature = "pure-rust")]
fn embed_signature_into_prepared_pure(
    prepared_code: &[u8],
    signature: &[u8],
    sig_offset: usize,
...
/// Build a complete SuperBlob signature using pure Rust cryptography.
#[cfg(feature = "pure-rust")]
#[allow(clippy::too_many_arguments)]
fn build_superblob_pure(
    code: &[u8],
    slice: &ArchSlice,
    identifier: &str,
    team_id: Option<&str>,
    ...
    let cd_sha1 = build_code_directory_pure(
    ...
    let cd_sha256 = build_code_directory_pure(
        identifier, team_id, code, slice, entitlements,
    ...
    let cms_data = cms_pure::sign_with_apple_attrs(
        &cd_sha1,
        &signing_key,
        &signing_cert,
        &cert_chain,
...
/// Build a CodeDirectory blob with the specified hash type (pure Rust version).
#[cfg(feature = "pure-rust")]
fn build_code_directory_pure(
    identifier: &str,
    team_id: Option<&str>,
    code: &[u8],
    slice: &ArchSlice,
...
/// Convert PureRustCredentials to types needed by cms_pure::sign_with_apple_attrs
#[cfg(feature = "pure-rust")]
...
/// Extract the Common Name (CN) from a certificate's subject (pure Rust version).
#[cfg(feature = "pure-rust")]
fn extract_subject_cn_pure(cert: &x509_certificate::X509Certificate) -> Option<String> {
...
mod tests {
    ...
    #[test]
    fn test_sha1_hash_deterministic_pure() {
    ...
    #[test]
    fn test_sha256_hash_deterministic_pure() {
        let data = b"test data for hashing";
